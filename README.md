...
`identity` (including Profile, Emergency Contacts, Device Tokens), `location`, `historicaldata`, `risk`, `sos`, `incident`, `notification` (stub), `emergencyservices` (stub), and `admin` are the backend module boundaries. (There is no separate `profile` package; profile functionality resides inside `com.geoshield.identity`). Modules communicate through service interfaces.
...
- IMPLEMENTED: Baseline Contextual Risk Engine — deterministic weighted scoring, explainability, safety recommendations, and `RiskScores` audit persistence. 4 live factors (Historical 0.30, Weather 0.20, Time-of-Day 0.15, User Reports 0.10 = 0.75 live weight) and 3 unavailable factors (0.25 weight, contributing 0.0: Service Proximity 0.15, Connectivity 0.05, Other Context 0.05). Maximum attainable baseline risk score is 75; the Critical band (80–100) is currently unreachable under the baseline model. It does not synthesize unavailable contextual data.
...
- Stub only (marker interfaces / pass-through skeletons exist, no functional implementation): Notifications (`NotificationService`), Emergency Services (`EmergencyServicesService`), Offline Sync (`LocalDatabase`, `SyncWorker`), Rate limiting (`RateLimitFilter` pass-through skeleton filter; no rate limiting enforced, no 60 req/min limit exists).
