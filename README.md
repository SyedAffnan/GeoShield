...
- IMPLEMENTED: Admin module — system statistics, user management, user provisioning (`RESPONDER`/`ADMIN`), account activation/deactivation, system-wide incident oversight. Development accounts seeded via `DataInitializer`.
...
- IMPLEMENTED: Baseline Contextual Risk Engine — deterministic weighted scoring, explainability, safety recommendations, and `RiskScores` audit persistence. 5 live factors (Historical 0.30, Weather 0.20, Time-of-Day 0.15, Emergency Service Proximity 0.15, User Reports 0.10 = 0.90 live weight) and 2 unavailable factors (0.10 weight, contributing 0.0: Connectivity 0.05, Other Context 0.05). Maximum attainable baseline risk score is 90.0; the Critical band (80–100) is reachable for scores $\ge 80.0$. It does not synthesize unavailable contextual data.
