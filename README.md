...
- Implemented: Identity/Auth, Tourist Profile, Emergency Contacts, Device Tokens, and Location. Refresh tokens are **partially implemented** (minted and stored BCrypt-hashed on the user record at login; no `/api/v1/auth/refresh` redemption or rotation endpoint exists).
...
- IMPLEMENTED: SOS module — one-tap emergency alert with full status lifecycle (`PENDING`→`ACKNOWLEDGED`→`RESPONDING`→`RESOLVED`, `CANCELLED`); tourist and responder endpoints. SOS does **not** trigger push notifications (Notification module is a stub; SOS-to-notification integration is not implemented).
...
- Not implemented: Random Forest production integration (offline research only) and Maps SDK live integration (mobile uses UI placeholder). Geographic resolution is at State/UT granularity only; city, district, and sub-district resolution are not implemented.
...
- Database Schema Management: Development and local environments currently use Hibernate `ddl-auto: update`. A formal versioned migration strategy (e.g. Flyway or Liquibase) remains a future infrastructure decision. No Flyway or Liquibase migration scripts or dependencies are present in the codebase.
