...
- Stub only (marker interfaces / pass-through skeletons exist, no functional implementation): Notifications (`NotificationService`), Offline Sync (`LocalDatabase`, `SyncWorker`), Rate limiting (`RateLimitFilter` pass-through skeleton filter; no rate limiting enforced, no 60 req/min limit exists).
