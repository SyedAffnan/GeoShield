...
`identity` (including Profile, Emergency Contacts, Device Tokens), `location`, `historicaldata`, `risk`, `sos`, `incident`, `notification` (stub), `emergencyservices`, and `admin` are the backend module boundaries. (There is no separate `profile` package; profile functionality resides inside `com.geoshield.identity`). Modules communicate through service interfaces.
