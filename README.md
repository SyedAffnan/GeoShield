...
`identity` (including Profile, Emergency Contacts, Device Tokens), `location`, `historicaldata`, `risk`, `sos`, `incident`, `notification` (stub), `emergencyservices`, `news`, and `admin` are the backend module boundaries. (There is no separate `profile` package; profile functionality resides inside `com.geoshield.identity`). Modules communicate through service interfaces.

## Implementation status

- Implemented: Identity/Auth, Tourist Profile, Emergency Contacts, Device Tokens, and Location. Refresh tokens are **partially implemented** (minted and stored BCrypt-hashed on the user record at login; no `/api/v1/auth/refresh` redemption or rotation endpoint exists; rotation endpoint is deferred).
- IMPLEMENTED AND VERIFIED: Historical Data Processing, `HistoricalSafetyRecords` persistence, and District-Ready Architecture (Step 5). The batch pipeline normalizes the approved MoRTH Road Accidents in India 2024 Annexure-4 CSV and NCRB Crime in India 2023 Table 13A.2 CSV into one aggregate-statistics table. The schema models hierarchical levels (`NATIONAL`, `STATE_UT`, `DISTRICT`, `CITY`) with canonical `parent_unit` links (`NATIONAL` -> `'India'`, `STATE_UT` -> `'India'`, `DISTRICT` -> parent State/UT). A 7-column unique constraint `uk_historical_safety_record_hierarchy` (`source`, `source_year`, `geographic_level`, `parent_unit`, `geographic_unit`, `category`, `metric_name`) guarantees deduplication and is verified at startup by `HistoricalSafetyRecordSchemaMigrator`. Contains exactly 286 verified rows (71 National, 215 State/UT, 0 District, 0 City, zero nulls/blanks). Universal State/UT Baseline Fallback is implemented via `HistoricalDataFallbackService`; district evaluation is disabled by default in production (`district-evaluation-enabled: false`).
- IMPLEMENTED AND VERIFIED: SOS module and SOS Reliability (Step 3) — one-tap emergency alert with full status lifecycle (`PENDING`→`ACKNOWLEDGED`→`RESPONDING`→`RESOLVED`, `CANCELLED`); tourist and responder endpoints. Reliability hardened via `SosTransactionHelper` using `REQUIRES_NEW` transaction boundary, `client_request_id` unique indexing for idempotency deduplication, race-condition conflict recovery via `resolveExistingSosAfterConflict`, and resilient cancellation. SOS does **not** trigger push notifications (Notification module is a stub; SOS-to-notification integration is not implemented).
- IMPLEMENTED: Admin module — system statistics, user management, user provisioning (`RESPONDER`/`ADMIN`), account activation/deactivation, system-wide incident oversight. Development accounts seeded via `DataInitializer` (profile-gated to non-production environments via `@Profile("!prod")` and `geoshield.dev-seed.enabled`).
- IMPLEMENTED: Emergency Responder module — incident queue with status transitions, SOS queue with response workflow.
- IMPLEMENTED AND VERIFIED: Incident reporting, authenticated owner-scoped incident reads, responder incident queue and status transitions, request idempotency (unique index on `client_request_id`), and SHA-256 record-level integrity verification (`IncidentIntegrityHasher`). When an incident is created, a 64-character SHA-256 digest is generated with length-prefixed framing over six immutable fields (`reporterId`, `incidentType`, `description`, canonicalized `latitude`, canonicalized `longitude`, and `clientRequestId`) and persisted in `incidents.integrity_hash` (`VARCHAR(64)`, `updatable = false`). The hash is re-verified using constant-time comparison on authorized reads and status updates; any in-storage alteration or bit corruption triggers an HTTP 409 Conflict rejection. (Provides record-level tamper evidence for incident reports; does not implement blockchain, hash chains, digital signatures, or asymmetric PKI, and is not applied to SOS alerts or risk scores).
- IMPLEMENTED AND VERIFIED: Baseline Contextual Risk Engine & Decision Audit Traceability (Step 2) — deterministic weighted scoring, explainability, safety recommendations, and comprehensive `RiskScores` audit persistence. Every assessment persists a unique `decisionId` (UUID string); `decimalScore` is retained as a BigDecimal separately from the integer score and persisted in `decimal_score` with DECIMAL(5,2) precision/scale, while the integer score is rounded independently. Also captures `effectiveRiskLevel` (reflecting active SACHET disaster overrides), `overrideActive` flag, `sachetAlertIdentifier`, `geographicLevel`, `geographicUnit`, `stateCode` (`stcode` derived from Survey of India boundary resolution), `dataCompleteness` (JSON metadata audit documenting available vs missing inputs), and `factorDetails` (JSON snapshot of individual factor scores, weights, and availability). Tourist risk queries are strictly isolated to the requesting tourist (`findByUserIdOrderByCreatedAtDesc`). Persistence failures propagate cleanly without silent swallowing. Mobile parses `decisionId` and `dataCompleteness`, rendering a non-penalizing `_DataCompletenessNoticeCard` for degraded contexts. Evaluates 7 configured factors, of which 5 are currently live (0.90 total weight: Historical 0.30, Weather 0.20, Time-of-Day 0.15, Emergency Service Proximity 0.15, User Reports 0.10) and 2 remain reserved/dormant (0.10 total weight: Connectivity 0.05, Other Context 0.05 contributing 0.0). Authoritative classification thresholds: Low (0–39), Medium (40–59), High (60–79), Critical (80–100). Maximum attainable baseline risk score is 90.0; the Critical band (80–100) is reachable for scores $\ge 80.0$. Unavailable contextual data is never synthesized.
- IMPLEMENTED AND VERIFIED: Historical Road Injury Rate Normalization & Provenance (Step 4 & Step 5) — Historical factor ($w=0.30$) normalizes MoRTH 2024 State/UT road injury rate using metric `"Total Number of Persons Injured in Road Accidents Per Lakh Population - 2024"`, formula: $\text{normalized} = \min\left(100.0, 100.0 \times \frac{\text{observedRate}}{152.6000}\right)$, where $152.6000$ is the empirical maximum per-lakh rate in the MoRTH 2024 dataset, observed specifically in Kerala (while the absolute accident count max is 71,493.0000 in Tamil Nadu). *Architectural Note:* 152.6000 is an empirical State/UT rate maximum and is NOT a valid or calibrated baseline for district evaluations; district normalization methodology remains **OWNER DECISION REQUIRED**. Full data provenance contract (Step 4, NFR-012) is implemented across all features via `NormalizedRiskFeature` (6 fields: `sourceType`, `sourceIdentifier`, `observedAt`, `freshnessSeconds`, `geographicScope`, `normalizationDetails`).
- IMPLEMENTED AND VERIFIED: In-Process Algorithmic & Service Microbenchmarks (Step 6) — isolated performance microbenchmarks (`RiskEngineBenchmarkTest` measuring internal risk fusion execution latency; `SosReliabilityBenchmarkTest` measures mocked in-process SOS service-layer dispatch, idempotency, conflict-recovery, and cancellation processing; 100 warmups, 500 measured iterations, floor-index percentiles). Benchmarks run in-process with mocks and do NOT measure external HTTP, Spring Security, MySQL, or network latency.
- IMPLEMENTED AND VERIFIED: Incident risk factor (User Reports) — `IncidentRiskFeatureService` evaluates active stored incident reports within a 10 km spatial radius (Haversine distance with linear decay $1 - d/10$) and 24-hour temporal window (linear recency decay $1 - \Delta t/24$). Severity tiers derive from `incidentType` (Assault: 100, Theft: 75, Accident: 50, Hazard: 25; default 50) and status multipliers (`REPORTED`/`ACKNOWLEDGED`: 1.0, `RESPONDING`: 0.8, `RESOLVED`/`CANCELLED`: 0.0). Results clamp to [0, 100]. When no qualifying incidents exist, it reports an honest zero (available, contributing 0.0 risk points). If tourist coordinates are missing, it reports explicitly unavailable without fabrication.
- IMPLEMENTED AND VERIFIED: Emergency Service Proximity risk factor — `EmergencyServiceProximityRiskService` and `EmergencyServicesServiceImpl` evaluate physical facilities from an India-wide OSM-mapped reference dataset of 57,939 validated OSM-mapped emergency-service records across all 36 States and Union Territories of India (53,633 medical, 3,784 response unit/police, 522 fire stations) extracted from OpenStreetMap under ODbL 1.0 ("© OpenStreetMap contributors") via the Humanitarian OpenStreetMap Team (HOT) / Geofabrik on HDX (medical, police, fire) and Overpass API (police/response-unit supplementation only; Overpass fire query returned zero results), point-in-polygon validated against official state boundaries. Calculates nearest facility via Haversine distance, normalized linearly up to Dmax = 10.0 km ($f_{proximity} = \min(100.0, (d_{min}/10.0) \times 100.0)$), contributing up to 15.0 risk points (weight 0.15).
...
- IMPLEMENTED AND VERIFIED: Risk API — `GET /api/v1/risk` returns the authenticated tourist's current baseline risk using their stored current location (does not accept latitude/longitude query parameters).
- IMPLEMENTED AND VERIFIED: Dual-Zone Geofencing Engine — backend stateless `GeofencingService` and Flutter mobile `GeofencingEngine`, fixed 1,000 m Pre-Warning entry and 500 m Core entry, state-dependent exit hysteresis, per-hazard cooldown, local geofence notifications, and tourist-dashboard `GeofenceStatusCard`; test-only synthetic fixtures `TEST-HZ-001` and `TEST-HZ-002`; real-world black-spot production data ingestion remains deferred/blocked.
- IMPLEMENTED AND VERIFIED: SACHET Disaster Alert Override Architecture (Option A) — dedicated high-priority alert and risk override channel ingesting NDMA SACHET CAP 1.2 XML feeds via canonical endpoint `POST /api/v1/alerts/sachet` (admin-only). Features XXE-hardened parsing (`SachetCapXmlParser`), 512 KB request payload bounding, database unique constraint on `(identifier, sender)`, space- and comma-delimited circle geometry normalization, polygon validation ($\ge 3$ vertices with inclusive edge/vertex handling), multi-area geometry aggregation (`MULTI`), deduplication, monotonic cancellation, CAP 1.2 `<references>` cancellation propagation, status gating (`Actual`, or `Test`/`Exercise` when `isSynthetic=true`), CRITICAL emergency override on qualifying severe alerts (`Extreme`/`Severe` + `Immediate`/`Expected`), location snapshot reuse via `RiskAssemblyContext` (avoiding redundant lookups), strict baseline score preservation without formula tampering, active alert listing via `GET /api/v1/alerts/sachet/active`, and tourist dashboard presentation via `SachetDisasterAlertCard` with synthetic watermark `[TEST FIXTURE / SYNTHETIC]`.
- IMPLEMENTED AND VERIFIED: Recent Safety Event / News Intelligence module (`com.geoshield.news` and Flutter `features/news`) — location-based safety news intelligence with a provider-agnostic SPI (`NewsProvider`), GNews API implementation (`GNewsProvider`), thread-safe mock provider with synthetic fixtures (`MockNewsProvider`), deterministic rule-based keyword & relevance classifier (`EventClassifier`: 7 categories, 5 severities), headline/snippet locality relevance filter (`LocationRelevanceFilter`), temporal + Jaccard token overlap deduplication (`EventDeduplicationService`), in-memory ConcurrentHashMap cache with 45m TTL and 200 max size (`NewsServiceImpl`), and endpoint `GET /api/v1/news/recent`. Integrated into Flutter tourist dashboard via `RecentSafetyEventsCard` with loading, error, provider unavailable, empty, and populated states, Material 3 design, full attribution without coordinate fabrication, and article URL copy action. Strictly informational; zero modification to numeric risk score, weights, thresholds, or SACHET overrides.
- Not implemented: Random Forest production integration (offline research only) and Maps SDK live integration (mobile uses UI placeholder). Production district dataset & boundary mapping are deferred.
- Stub only (marker interfaces / pass-through skeletons exist, no functional implementation): Push Notification delivery (`NotificationService` marker interface; SACHET disaster alerts are implemented), Offline Sync (`LocalDatabase`, `SyncWorker`), Rate limiting (`RateLimitFilter`): rate limiting remains a pass-through stub. No request limit is currently enforced because endpoint-specific thresholds have not been formally approved.
- Database Schema Management: Development and local environments currently use Hibernate `ddl-auto: update` with additive automated migrators (`HistoricalSafetyRecordSchemaMigrator`). A formal versioned migration strategy (e.g. Flyway or Liquibase) remains a future infrastructure decision. No Flyway or Liquibase migration scripts or dependencies are present in the codebase.
- Transport security & Credential hygiene: Production requires HTTPS (`usesCleartextTraffic="false"` in main AndroidManifest, remote HTTP origins rejected by mobile `ApiClient`); local development permits HTTP loopback origins and debug build variant. Default accounts in `DataInitializer` are non-production only; SQL statement logging (`show-sql`) defaults to `false`.
...
- **Backend Test Suite:**
  ...
  mvn test
  ```
  **Result:** `BUILD SUCCESS`. Total tests run: 637 (637 passed, 0 failures, 0 errors, 0 skipped).
  Targeted MySQL Integration Tests (11/11 passed when MySQL is configured):
  ```bash
  mvn test "-Dtest=HistoricalSafetyRecordMySQLTest,DistrictHistoricalSafetyRecordRepositoryTest"
  ```
- **Mobile Test Suite:**
  ```bash
  cd mobile
  ...
  flutter test
  ```
  **Result:** `flutter analyze` reports no issues (`No issues found!`); `flutter test` reports **171/171 tests passed**.
- **In-Process Microbenchmarks:**
  ```bash
  mvn test -Dtest=RiskEngineBenchmarkTest,SosReliabilityBenchmarkTest
  ```
  **Result:** 100 warmups, 500 measured iterations, p50/p95/p99 latency reported for internal algorithmic risk fusion and mocked in-process SOS service-layer dispatch, idempotency, conflict-recovery, and cancellation processing (does not measure real MySQL persistence, locking, database concurrency, or network latency).
